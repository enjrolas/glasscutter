...
void progressBar(int current, int max, int x, int y, int barWidth, int barHeight, int startTime)
{
  stroke(0, 0, 255);
  fill(0, 0, 255);
...
void sliceModel()
{
  ...
  x=float(cp5.get(Textfield.class, "x").getText());
  y=float(cp5.get(Textfield.class, "y").getText());
  z=float(cp5.get(Textfield.class, "z").getText());
  thickness=float(cp5.get(Textfield.class, "sliceThickness").getText());
  microslices=int(cp5.get(Textfield.class, "microslices").getText());
  ...
  float pageWidth=float(cp5.get(Textfield.class, "pageWidth").getText());
  float pageHeight=float(cp5.get(Textfield.class, "pageHeight").getText());
  float spacing=float(cp5.get(Textfield.class, "spacing").getText());
  float pageMargin=float(cp5.get(Textfield.class, "pageMargin").getText());
  int rows=int(cp5.get(Textfield.class, "rows").getText());
  int columns=int(cp5.get(Textfield.class, "columns").getText());
  int resolution=int(cp5.get(Textfield.class, "resolution").getText());
  float overlap=float(cp5.get(Textfield.class, "overlap").getText());
...
class Slicer {
  ...
  int startTime;
  PFont font;
  float margins=.05;  //5% margins
  float scale=0.352;  //1 pixel in a pdf, scaled to 100%, will print as 0.352 mm
  int totalSlices;
  int weight=1;
  int slice=0;
  int currentSlice=0;
  ...
  void savePDF(float pageWidth, float pageHeight, int _rows, int _columns, float _pageMargin, float _spacing)
  {
  ...
  {    
    ...
    float scalingFactor=(float)resolution/25.4;
  ...
  void drawCrosshatches(PGraphics pdf, int x, int y, int w, int h, int length, int slice)
  {
    pdf.fill(0);
    pdf.strokeWeight(1);
    pdf.stroke(0);
        ...
        if (lines[0].toLowerCase().equals("ply"))  //the first line in the file should say 'ply'
        {
          ...
          while (!lines[index].toLowerCase ().equals("end_header")) //go through the end of the header
          {
            ...
            if (parts[0].equals("element"))
            {
              ...
              propertyIndex=index;
              elements.add(new Element(parts[1], int(parts[2])));
            }
          ...
          for (int i=0;i<elements.size();i++)
          {
            if (((Element)elements.get(i)).name.equals("vertex"))
            {
              ...
              for (int j=0;j<((Element)elements.get(i)).units;j++)
              {
                parts=split(lines[index], ' ');
                if (parts.length==3)
                  vertices.add(new Vertex(float(parts[0]), float(parts[1]), float(parts[2])));
                else if (parts.length>=6)
                  vertices.add(new Vertex(float(parts[0]), float(parts[1]), float(parts[2]), int(parts[redIndex]), int(parts[greenIndex]), int(parts[blueIndex])));
                index++;
              }
            }
            else if (((Element)elements.get(i)).name.equals("face"))
            {
              for (int j=0;j<((Element)elements.get(i)).units;j++)
              {
                parts=split(lines[index], ' ');
                if (parts.length==4)
                  triangles.add(new Triangle(int(parts[1]), int(parts[2]), int(parts[3])));
                else if (parts.length==5)
                {
                  triangles.add(new Triangle(int(parts[1]), int(parts[2]), int(parts[3])));
                  triangles.add(new Triangle(int(parts[2]), int(parts[3]), int(parts[4])));
                }
  ...
  float getAxis(PVector vector, char axis)
  {
    switch(axis)
    {
      case('x'):
  ...
  void sliceModel(float thickness, char axis)
  {
    status="slicing";
    startTime=millis();
    slices.clear(); 
